...
BEGIN
    ...
    /*
        Find the union of this update plus any other updates with the
        same prefix, and update the repos_merkle table accordingly.
    ...
    */
    ...
    FROM
        ...
        ON
            repo_related_updates.update_id = updates.id
        INNER JOIN
            repo_related_updates up2
        ON
            up2.update_id = NEW.id AND
            up2.repo_id = repo_related_updates.repo_id
        WHERE
            updates.prefix = NEW.prefix
        ORDER BY
            repo_related_updates.repo_id,
