...
multiple times during development and for security updates, and each
...
We assume that there are relatively few project updates in comparison
to task and issue updates.
...
There is a table for "updates" that represent changes, but that table
...
=item * Updates are "resolved" by inserting a row into
func_merge_updates that calculates the updates.uuid SHA1 hash. If
updates.uuid is already set (i.e. it was passed in from another
...
project_deltas and *_status_deltas in the "hub_related_updates" table.
...
When a local user registers a hub all project-only details are
imported.  Initially this so that the local user can see what projects
are available on the hub. Therefore, for each hub we track the related
project_deltas and *_status_deltas in the "hub_related_updates" table.

Current state of topics, table for updates to topics, tables to track
...
simbly exchange a certain set of updates to a certain set of nodes on
demand. The state of a particular node is the result of the subset of
updates it has, and it doesn't care what the other nodes are doing.
...
When a local user registers a hub all project-only details are imported
from the "hub_related_updates" table.  Initially this so that the local
...
The first part of a project export is a local operation. The local
project-only updates are added to hub_related_updates. The next time a
hub sync happens the details will be transferred across.

=begin graph-easy

    graph { flow: east; }

    [ local ] <-- hub_related_updates --> [ hub ]
...
The second part of an export is to update the project to set its hub_id
values.

The third part of an export is the synchronisation of the
project_related_updates on both sides. However given that the hub
doesn't have anyting in there yet this is effectively a one-way copy.

=head2 Project Import

We already have project-only updates for $PROJECT in the database. We
may also have issue updates related to $PROJECT locally.
...
hub-related updates (which are actually project meta-data) are
exchanged, then the project-related updates (which are actually task
and issue data) are exchanged.

=head3 Hub Related Updates Synchronisation

Syncing hub-related updates is relatively easy. The
hub_related_updates_merkle table from each end is recursively compared
top down (or breadth-first? - need to check) to discover the
differences, from which a set of updates is stored in a temporary
table. The updates are then tranferred to the other end in the order
they were created in the database.

=head3 Project Related Updates Synchronisation

Syncing project-related updates starts out the same way as hub-related
update synchronisation.  The project_related_updates_merkle table from
each end is recursively compared top down (or breadth-first? - need to
check) to discover the differences.

However, the set of updates which are stored at each end to be
...
The updates are then tranferred to the other end in the order they were
created in the database.

=head2 Merging Deltas

One of the actions that takes place when an update is resolved is the
...
result according to the update timestamp wins.
