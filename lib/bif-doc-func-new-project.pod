...
=item * A row is inserted into F<hub_deltas> against update #2 for the
...
=item * A row is inserted into hub_related_updates_merkle taking into account the new
update #2.
...
=item * A row is inserted into F<project_deltas> against update #1, which kicks off the
...
=item * A row is inserted into project_related_updates_merkle taking into account the new
update #1.
...
L<project_deltas|bif-doc-table-project-updates> resulting in
