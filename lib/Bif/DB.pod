...
Looks up the project identified by C<$PATH> and returns undef, a
negative integer, or a hash reference containg the following keys:
...
C<$PATH> can be a plain C<PATH> or a C<PATH@ALIAS>. If more than one
match is found then the negative number of matches is returned.
